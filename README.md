...
AI-powered mobile app that scans ICAR Soil Health Cards using Groq Vision OCR and delivers crop-specific fertilizer recommendations with cost estimates in Hindi and English. Includes NPK deficiency detection and a 0–100 soil health scoring system, backed by a Node.js/Express + PostgreSQL backend with OTP auth and Firebase push notifications.
...
Cross-platform mobile app supporting end-to-end booking workflows, secure Stripe payments, and automated KYC via REST APIs. Backend built with JWT authentication and optimized MongoDB schemas, reducing query response time by 30% under concurrent load.
...
Clinical Machine Learning Pipeline for smartphone-based Photoplethysmography (PPG) signal acquisition, DSP filtering, and quality grading using Random Forests. Engineered a full-stack system extracting raw optical signals via custom Kotlin bridges, passing them through a Python backend with 4th-order Butterworth filters and Mobile Domain Calibration.
...
Full-stack Retrieval-Augmented Generation (RAG) platform to automate legal document analysis using vector-based semantic search to extract critical clauses and liabilities. NLP pipeline built with ChromaDB and SentenceTransformers, integrating Llama-3.3 to generate automated risk scores.
...
XGBoost-based clinical prediction model with SHAP explainability on 11 biomarkers, deployed via an interactive Streamlit dashboard. Developed an ML model with SHAP-based per-patient explainability and automated PDF health reports.
...
Production-ready MERN-stack food delivery platform with JWT authentication, Stripe payment integration, and complete order lifecycle management. Built a full-stack platform with optimized API endpoints and MongoDB schemas supporting high concurrent order volumes.
...
- 🎓 **Certificate:** Artificial Intelligence & Machine Learning Training — Infosys Springboard (2024)
- 🎓 **Certificate:** Job-Ready MERN Full-Stack Web Development — 30 Days Coding (Nov 2024)
- 🏆 **Hackathon Finalist:** IDEA-FIESTA Hackathon (2022)
